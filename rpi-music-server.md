...
- **Mopidy pipeline watchdog** — `mopidy-pipeline-watchdog.service` tails the Mopidy journal and, on `Internal data stream error`, issues an RPC `core.playback.stop` then `core.playback.play` to rebuild the gstreamer pipeline. Without this, playback state stays `"playing"` but no audio reaches the speaker until manual intervention. Rate-limited to 3 fixes per 5 minutes so a genuinely broken track / persistent fault is surfaced rather than masked. Logs to syslog with tag `mopidy-watchdog`.
